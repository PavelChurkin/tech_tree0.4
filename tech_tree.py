...
json_path = os.path.join(BASE_DIR, 'techno7.json')
try:
...
file_menu.add_command(label="Сохранить прогресс", command=lambda: save_progress())
file_menu.add_command(label="Загрузить прогресс", command=lambda: load_progress())
...
sort_menu.add_command(label="По статусу", command=lambda: sort_by_status())
sort_menu.add_command(label="По алфавиту", command=lambda: sort_by_alphabet())
...
help_menu.add_command(label="Открыть справку (F1)", command=lambda: open_help())
help_menu.add_command(label="Подробности (F2)", command=lambda: open_mhtml())

# Привязка клавиши F1 к справке
root.bind('<F1>', lambda e: open_help())
root.bind('<F2>', lambda e: open_mhtml())
...
def open_help() -> None:
    ...
    if os.path.exists(fr"web\help.mhtml"):
        webbrowser.open(fr"web\help.mhtml")
    else:
